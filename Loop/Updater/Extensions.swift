...
extension Int {
    var daysToSeconds: Double { Double(self) * 86400 }
}

func printOS(_ items: Any..., separator: String = " ", terminator _: String = "\n") {
    os_log("%@", log: .default, type: .default, items.map { "\($0)" }.joined(separator: separator))
}

func updateOnMain(after delay: Double? = nil, _ updates: @escaping () -> ()) {
...
func updateOnBackground(_ updates: @escaping () -> ()) {
    DispatchQueue.global(qos: .userInitiated).async(execute: updates)
}

func checkAppDirectoryAndUserRole(
    completion: @escaping ((isInCorrectDirectory: Bool, isAdmin: Bool)) -> ()
) {
...
func isCurrentUserAdmin(completion: @escaping (Bool) -> ()) {
    let process = Process()
...
func killApp(appId: String, completion: @escaping () -> () = {}) {
    NSWorkspace.shared.runningApplications.first(where: { $0.bundleIdentifier == appId })?.terminate()
    completion()
}

func ensureApplicationSupportFolderExists(appState _: AppState) {
